...
import { useLocation } from 'react-router-dom' // useNavigate only in v6 of react router
...
const withAuth = Component => props => {
  ...
  useEffect( ()=> {
    if (auth && auth.authenticated) {
      console.log('authenticated hoc effect..')
    }
  }, [auth])
  ...
  console.log('withAuth: component is not authenticated, access not allowed, resetting auth status and redirecting to sign-in!!', auth)
